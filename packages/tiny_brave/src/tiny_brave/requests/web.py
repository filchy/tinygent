...
class WebSearchRequest(BaseSearchRequest):
    ...
    text_decorations: bool = Field(
        default=True,
        description='Whether snippets include decoration markers.'
    )

    result_filter: str | None = Field(
        default=None,
        description=(
            'Comma-delimited result types to include '
            "(e.g. 'discussions,videos,web')."
        )
    )

    goggles_id: str | None = Field(
        default=None,
        description='Deprecated: use `goggles`. Goggles ID string.'
    )
    ...
    units: Literal['metric', 'imperial'] | None = Field(
        default=None,
        description='Measurement system. Default inferred from country.'
    )
    ...
    summary: bool | None = Field(
        default=None,
        description='If true, enables summary key generation in results.'
    )
